      ...
      <div class="download-container">
        ...
        <div class="download-cards">
          ...
          <div v-for="card in sortedCards" :key="card.platform" class="download-card">
            ...
            <h2>{{ card.platform }}</h2>
            <p class="version">{{ t('download.version', { version: DESKTOP_RELEASE_VERSION }) }}</p>
            <a :href="card.url" class="download-btn"> {{ t('download.downloadFor', { platform: card.platform }) }} </a>
...
const cards = computed(() => [
  ...
  },
  {
    ...
    req: t('download.windowsReq')
  }
])

const sortedCards = computed(() => {
  if (isMac) return cards.value
  return [cards.value[1], cards.value[0]]
})
</script>
